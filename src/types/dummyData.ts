// const userData = {
//   userId: localUser.email,
//   userName: localUser.displayName,
//   nickname: localUser.displayName,
//   userAvatar: localUser.photoURL,
//   address: "",
//   phone: "",
//   orders: [],
//   sales: {
//     saleId: `saleId_${localUser.uid}`,
//     salesItems: [],
//   },
//   carts: {
//     cartId: `cartId_${localUser.uid}`,
//     cartsItems: [],
//   },
//   wishlists: [],
// };
export const defaultImage = 'https://via.placeholder.com/150'

export const userData = [
  {
    userId: "userId_119M",
    userName: "Sumner",
    nickname: "Tooth",
    userAvatar:
      "https://robohash.org/estharumlaudantium.png?size=70x70&set=set1",
    address: "4923 Mesta Park",
    phone: "010-2436-4062",
    orders: [
      {
        orderId: "orderId_754c",
        totalPrice: 0,
        items: [
          {
            itemId: "itemId850H",
            itemName: "NASDAQ",
            price: 602000,
            size: "M",
            imageUrl: "https://i.postimg.cc/K89Dxv30/1-1.webp",
            quantity: 1,
          },
          {
            itemId: "itemId534M",
            itemName: "NASDAQ",
            price: 36000,
            size: "XS",
            imageUrl: "https://i.postimg.cc/KjFMtdgK/2-1.webp",
            quantity: 1,
          },
        ],
        buyerInfo: {
          name: "Sumner",
          address: "4923 Mesta Park",
          phone: "010-2436-4062",
        },
        paymentMethod: "creditcard",
        orderDate: "2024-01-28",
      },
      {
        orderId: "orderId_578e",
        totalPrice: 0,
        items: [
          {
            itemId: "itemId534M",
            itemName: "NASDAQ",
            price: 36000,
            size: "XS",
            imageUrl: "https://i.postimg.cc/KjFMtdgK/2-1.webp",
            quantity: 1,
          },
          {
            itemId: "itemId850H",
            itemName: "NASDAQ",
            price: 602000,
            size: "M",
            imageUrl: "https://i.postimg.cc/K89Dxv30/1-1.webp",
            quantity: 1,
          },
        ],
        buyerInfo: {
          name: "Sumner",
          address: "4923 Mesta Park",
          phone: "010-2436-4062",
        },
        paymentMethod: "creditcard",
        orderDate: "2024-07-28",
      },
    ],
    sales: {
      salesId: "saleId_thdud01",
      salesItems: [
        {
          itemId: "itemId850H",
          itemName: "NASDAQ",
          price: 602000,
          size: "M",
          imageUrl: "https://i.postimg.cc/K89Dxv30/1-1.webp",
          quantity: 1,
          createdAt: "2024-06-17",
          isSales: false,
          options: ["배송비포함", "새상품"],
        },
        {
          itemId: "itemId111O",
          itemName: "NYSE",
          quantity: 2,
          size: "XS",
          price: 15000,
          imageUrl: "https://i.postimg.cc/Wz2xx1ks/4-1.png",
          createdAt: "2024-06-17",
          isSales: true,
          options: ["배송비포함", "새상품"],
        },
      ],
    },
    carts: {
      cartsId: "cartId_thdud_01",
      cartsItems: [
        {
          itemId: "itemId850H",
          itemName: "NASDAQ",
          price: 602000,
          size: "M",
          imageUrl: "https://i.postimg.cc/K89Dxv30/1-1.webp",
          quantity: 1,
        },
        {
          itemId: "itemId534M",
          itemName: "NASDAQ",
          price: 36000,
          size: "XS",
          imageUrl: "https://i.postimg.cc/KjFMtdgK/2-1.webp",
          quantity: 1,
        },
        {
          itemId: "itemId784I",
          itemName: "NYSE",
          price: 226000,
          size: "L",
          imageUrl: "https://i.postimg.cc/cLrGsC5H/3-1.webp",
          quantity: 1,
        },
      ],
    },
    wishlists: [
      {
        itemId: "itemId850H",
        itemName: "NASDAQ",
        price: 602000,
        imageUrl: "https://i.postimg.cc/K89Dxv30/1-1.webp",
      },
      {
        itemId: "itemId534M",
        itemName: "NASDAQ",
        price: 36000,
        imageUrl: "https://i.postimg.cc/KjFMtdgK/2-1.webp",
      },
    ],
  },
  {
    userId: "userId_208O",
    userName: "Hartwell",
    nickname: "Acott",
    userAvatar:
      "https://robohash.org/hicprovidentcupiditate.png?size=70x70&set=set1",
    address: "4 Susan Drive",
    phone: "010-6194-7197",
    orders: [],
    sales: {
      salesId: "saleId_208O",
      salesItems: [],
    },
    carts: {
      cartsId: "cartId_208O",
      cartsItems: [],
    },
    wishlists: [],
  },
  {
    userId: "userId_753m",
    userName: "Ronnica",
    nickname: "Gullen",
    userAvatar:
      "https://robohash.org/oditquisreiciendis.png?size=70x70&set=set1",
    address: "6204 Oneill Trail",
    phone: "010-3637-6171",
    orders: [],
    sales: {
      salesId: "saleId_753m",
      salesItems: [],
    },
    carts: {
      cartsId: "cartId_753m",
      cartsItems: [],
    },
    wishlists: [],
  },
  {
    userId: "userId_398G",
    userName: "Ingamar",
    nickname: "Scrange",
    userAvatar:
      "https://robohash.org/occaecatitemporaest.png?size=70x70&set=set1",
    address: "87 Spaight Junction",
    phone: "010-0806-1103",
    orders: [],
    sales: {
      salesId: "saleId_398G",
      salesItems: [],
    },
    carts: {
      cartsId: "cartId_398G",
      cartsItems: [],
    },
    wishlists: [],
  },
  {
    userId: "userId_020q",
    userName: "Emmy",
    nickname: "Coetzee",
    userAvatar: "https://robohash.org/etnumquamut.png?size=70x70&set=set1",
    address: "31 Acker Crossing",
    phone: "010-8367-1533",
    orders: [],
    sales: {
      salesId: "saleId_020q",
      salesItems: [],
    },
    carts: {
      cartsId: "cartId_020q",
      cartsItems: [],
    },
    wishlists: [],
  },
];

// createdAt: "2024-06-17",  업로드 한 날짜 추가해야함
export const usedItems = [
  {
    itemId: "itemId850H",
    itemName: "NASDAQ",
    description:
      "Etiiam faucibus cursus urna. Ut tellus. Nulla ut erat id mauris vulputate elementum. Nullam varius. Nulla facilisi. Cras non velit nec nisi vulputate nonummy. Maecenas tincidunt lacus at velit. Vivamus vel nulla eget eros elementum pellentesque. Quisque porta volutpat erat. Quisque erat eros, viverra eget, congue eget, semper rutrum, nulla. Nunc purus. Phasellus in felis. Donec semper sapien a libero. Nam dui. Proin leo odio, porttitor id, consequat in, consequat ut, nulla. Sed accumsan felis. Ut at dolor quis odio consequat varius. Integer ac leo. Pellentesque ultrices mattis odio.",
    quantity: 2,
    imageArr: [
      "https://i.postimg.cc/DzJLhQTz/1-2.webp",
      "https://i.postimg.cc/nLy7hsFG/1-3.webp",
      "https://i.postimg.cc/26nWbz3R/1-4.webp",
    ],
    price: 602000,
    imageUrl: "https://i.postimg.cc/K89Dxv30/1-1.webp",
    isSales: false,
    size: "M",
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller6",
      userName: "bmenhenitt5",
      nickname: "chandscombe5",
      userAvatar:
        "https://robohash.org/quisconsequatureveniet.png?size=50x50&set=set1",
      address: "9 Bobwhite Avenue",
      phone: "010-5325-6919",
    },
    reviews: [
      {
        reviewId: "reviewId_9080u",
        reviewInfo: {
          userId: "userId_7158s",
          userName: "balmak0",
          userAvatar:
            "https://robohash.org/temporadolorumquos.png?size=70x70&set=set1",
          rating: 1,
          review:
            "Curabitur convallis. Duis consequat dui nec nisi volutpat eleifend. Donec ut dolor. Morbi vel lectus in quam fringilla rhoncus. Mauris enim leo, rhoncus sed, vestibulum sit amet, cursus id, turpis. Integer aliquet, massa id lobortis convallis, tortor risus dapibus augue, vel accumsan tellus nisi eu orci.",
          createdAt: "2024-05-30",
        },
      },
      {
        reviewId: "reviewId_0266b",
        reviewInfo: {
          userId: "userId_6554G",
          userName: "bohollegan0",
          userAvatar:
            "https://robohash.org/doloremautsit.png?size=70x70&set=set1",
          rating: 4,
          review:
            "Proin interdum mauris non ligula pellentesque ultrices. Phasellus id sapien in sapien iaculis congue. Vivamus metus arcu, adipiscing molestie, hendrerit at, vulputate vitae, nisl. Aenean lectus.",
          createdAt: "2024-03-06",
        },
      },
    ],
  },
  {
    itemId: "itemId534M",
    itemName: "NASDAQ",
    description:
      "Pellentesque at nulla. Suspendisse potenti. Cras in purus eu magna vulputate luctus. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Vivamus vestibulum sagittis sapien. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Etiam vel augue. Vestibulum rutrum rutrum neque. Aenean auctor gravida sem. Praesent id massa id nisl venenatis lacinia. Aenean sit amet justo. Morbi ut odio. Cras mi pede, malesuada in, imperdiet et, commodo vulputate, justo. In blandit ultrices enim. Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Proin interdum mauris non ligula pellentesque ultrices. Phasellus id sapien in sapien iaculis congue. Vivamus metus arcu, adipiscing molestie, hendrerit at, vulputate vitae, nisl. Aenean lectus. Pellentesque eget nunc. Donec quis orci eget orci vehicula condimentum.",
    quantity: 2,
    size: "XS",
    price: 36000,
    imageUrl: "https://i.postimg.cc/KjFMtdgK/2-1.webp",
    imageArr: [
      "https://i.postimg.cc/x17N6kzV/2-2.webp",
      "https://i.postimg.cc/cHGgNVhD/2-3.webp",
      "https://i.postimg.cc/2yz11hPh/2-4.webp",
    ],
    isSales: true,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller1",
      userName: "ahellyer0",
      nickname: "aclawsley0",
      userAvatar: "https://robohash.org/autemautemvel.png?size=50x50&set=set1",
      address: "70140 Sunnyside Plaza",
      phone: "010-8433-9597",
    },
    reviews: [
      {
        reviewId: "reviewId_5251A",
        reviewInfo: {
          userId: "userId_2741F",
          userName: "kchainey1",
          userAvatar:
            "https://robohash.org/temporadolorumquos.png?size=70x70&set=set1",
          rating: 1,
          review:
            "In blandit ultrices enim. Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Proin interdum mauris non ligula pellentesque ultrices. Phasellus id sapien in sapien iaculis congue. Vivamus metus arcu, adipiscing molestie, hendrerit at, vulputate vitae, nisl. Aenean lectus.",
          createdAt: "2024-04-03",
        },
      },
      {
        reviewId: "reviewId_2306z",
        reviewInfo: {
          userId: "userId_6115t",
          userName: "kdanihel2",
          userAvatar:
            "https://robohash.org/etquisofficiis.png?size=70x70&set=set1",
          rating: 1,
          review:
            "Morbi ut odio. Cras mi pede, malesuada in, imperdiet et, commodo vulputate, justo. In blandit ultrices enim.",
          createdAt: "2024-06-27",
        },
      },
      {
        reviewId: "reviewId_8207b",
        reviewInfo: {
          userId: "userId_0625a",
          userName: "jcowansj",
          userAvatar:
            "https://robohash.org/explicaboeosdolor.png?size=70x70&set=set1",
          rating: 5,
          review:
            "Donec diam neque, vestibulum eget, vulputate ut, ultrices vel, augue. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Donec pharetra, magna vestibulum aliquet ultrices, erat tortor sollicitudin mi, sit amet lobortis sapien sapien non mi. Integer ac neque. Duis bibendum. Morbi non quam nec dui luctus rutrum.",
          createdAt: "2024-06-25",
        },
      },
      {
        reviewId: "reviewId_7253x",
        reviewInfo: {
          userId: "userId_2790W",
          userName: "mconrek",
          userAvatar:
            "https://robohash.org/velmolestiaevitae.png?size=70x70&set=set1",
          rating: 3,
          review:
            "In congue. Etiam justo. Etiam pretium iaculis justo. In hac habitasse platea dictumst. Etiam faucibus cursus urna.",
          createdAt: "2024-09-14",
        },
      },
      {
        reviewId: "reviewId_5533y",
        reviewInfo: {
          userId: "userId_3892Q",
          userName: "wknellerl",
          userAvatar:
            "https://robohash.org/earumdolorumnesciunt.png?size=70x70&set=set1",
          rating: 4,
          review:
            "Sed sagittis. Nam congue, risus semper porta volutpat, quam pede lobortis ligula, sit amet eleifend pede libero quis orci. Nullam molestie nibh in lectus. Pellentesque at nulla.",
          createdAt: "2024-05-31",
        },
      },
    ],
  },
  {
    itemId: "itemId784I",
    itemName: "NYSE",
    description:
      "Nulla ut erat id mauris vulputate elementum. Nullam varius. Nulla facilisi. Cras non velit nec nisi vulputate nonummy. Maecenas tincidunt lacus at velit. Vivamus vel nulla eget eros elementum pellentesque. Quisque porta volutpat erat. Quisque erat eros, viverra eget, congue eget, semper rutrum, nulla. Nunc purus. Phasellus in felis. Donec semper sapien a libero. Nam dui. Proin leo odio, porttitor id, consequat in, consequat ut, nulla. Sed accumsan felis. Ut at dolor quis odio consequat varius. Integer ac leo. Pellentesque ultrices mattis odio. Donec vitae nisi. Nam ultrices, libero non mattis pulvinar, nulla pede ullamcorper augue, a suscipit nulla elit ac nulla. Sed vel enim sit amet nunc viverra dapibus. Nulla suscipit ligula in lacus. Curabitur at ipsum ac tellus semper interdum. Mauris ullamcorper purus sit amet nulla.",
    quantity: 3,
    size: "L",
    price: 226000,
    imageUrl: "https://i.postimg.cc/cLrGsC5H/3-1.webp",
    imageArr: [
      "https://i.postimg.cc/DwHkmhtc/3-2.webp",
      "https://i.postimg.cc/DzGVXDh3/3-3.webp",
      "https://i.postimg.cc/W3cB6npF/3-4.webp",
    ],
    isSales: false,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller2",
      userName: "dpedlow1",
      nickname: "mcastaneda1",
      userAvatar:
        "https://robohash.org/minusmolestiaseligendi.png?size=50x50&set=set1",
      address: "49223 Debra Parkway",
      phone: "010-0766-4554",
    },
    reviews: [
      {
        reviewId: "reviewId_9213T",
        reviewInfo: {
          userId: "userId_0728E",
          userName: "fwoodnutt3",
          userAvatar:
            "https://robohash.org/nesciuntexquia.png?size=70x70&set=set1",
          rating: 1,
          review:
            "Morbi a ipsum. Integer a nibh. In quis justo. Maecenas rhoncus aliquam lacus. Morbi quis tortor id nulla ultrices aliquet.",
          createdAt: "2024-02-19",
        },
      },
      {
        reviewId: "reviewId_3895j",
        reviewInfo: {
          userId: "userId_4281V",
          userName: "sestevezm",
          userAvatar:
            "https://robohash.org/officiadoloreslaboriosam.png?size=70x70&set=set1",
          rating: 5,
          review:
            "Morbi porttitor lorem id ligula. Suspendisse ornare consequat lectus. In est risus, auctor sed, tristique in, tempus sit amet, sem. Fusce consequat. Nulla nisl. Nunc nisl. Duis bibendum, felis sed interdum venenatis, turpis enim blandit mi, in porttitor pede justo eu massa.",
          createdAt: "2024-08-16",
        },
      },
      {
        reviewId: "reviewId_6766E",
        reviewInfo: {
          userId: "userId_8885E",
          userName: "tschulern",
          userAvatar:
            "https://robohash.org/avelitlaborum.png?size=70x70&set=set1",
          rating: 2,
          review:
            "Mauris sit amet eros. Suspendisse accumsan tortor quis turpis. Sed ante. Vivamus tortor. Duis mattis egestas metus. Aenean fermentum. Donec ut mauris eget massa tempor convallis.",
          createdAt: "2024-03-09",
        },
      },
    ],
  },
  {
    itemId: "itemId111O",
    itemName: "NYSE",
    description:
      "Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.",
    quantity: 2,
    size: "XS",
    price: 15000,
    imageUrl: "https://i.postimg.cc/Wz2xx1ks/4-1.png",
    imageArr: [
      "https://i.postimg.cc/D0BRh3rL/4-2.png",
      "https://i.postimg.cc/7PVF00Fh/4-3.png",
      "https://i.postimg.cc/yY8wPWKP/4-4.png",
    ],
    isSales: true,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller3",
      userName: "gmidson2",
      nickname: "cyakob2",
      userAvatar: "https://robohash.org/quiaomnis.png?size=50x50&set=set1",
      address: "45966 Hoepker Plaza",
      phone: "010-9280-3918",
    },
    reviews: [
      {
        reviewId: "reviewId_5272g",
        reviewInfo: {
          userId: "userId_2261a",
          userName: "kmedcalfe4",
          userAvatar:
            "https://robohash.org/blanditiisautnumquam.png?size=70x70&set=set1",
          rating: 4,
          review:
            "Sed ante. Vivamus tortor. Duis mattis egestas metus. Aenean fermentum. Donec ut mauris eget massa tempor convallis.",
          createdAt: "2024-01-17",
        },
      },
      {
        reviewId: "reviewId_3705p",
        reviewInfo: {
          userId: "userId_3265c",
          userName: "lpretiouso",
          userAvatar:
            "https://robohash.org/errorquosinventore.png?size=70x70&set=set1",
          rating: 3,
          review:
            "Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Donec pharetra, magna vestibulum aliquet ultrices, erat tortor sollicitudin mi, sit amet lobortis sapien sapien non mi. Integer ac neque. Duis bibendum. Morbi non quam nec dui luctus rutrum. Nulla tellus.",
          createdAt: "2024-01-11",
        },
      },
      {
        reviewId: "reviewId_9669X",
        reviewInfo: {
          userId: "userId_5129D",
          userName: "lhodgin5",
          userAvatar: "https://robohash.org/adetmaxime.png?size=70x70&set=set1",
          rating: 4,
          review: "Fusce consequat.",
          createdAt: "2024-05-08",
        },
      },
    ],
  },
  {
    itemId: "itemId477P",
    itemName: "NASDAQ",
    description:
      "Integer pede justo, lacinia eget, tincidunt eget, tempus vel, pede. Morbi porttitor lorem id ligula. Suspendisse ornare consequat lectus. In est risus, auctor sed, tristique in, tempus sit amet, sem. Fusce consequat. Nulla nisl. Nunc nisl. Duis bibendum, felis sed interdum venenatis, turpis enim blandit mi, in porttitor pede justo eu massa. Donec dapibus. Duis at velit eu est congue elementum. In hac habitasse platea dictumst. Morbi vestibulum, velit id pretium iaculis, diam erat fermentum justo, nec condimentum neque sapien placerat ante. Nulla justo. Aliquam quis turpis eget elit sodales scelerisque. Mauris sit amet eros. Suspendisse accumsan tortor quis turpis. Sed ante. Vivamus tortor. Duis mattis egestas metus. Aenean fermentum. Donec ut mauris eget massa tempor convallis. Nulla neque libero, convallis eget, eleifend luctus, ultricies eu, nibh. Quisque id justo sit amet sapien dignissim vestibulum. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Nulla dapibus dolor vel est.",
    quantity: 3,
    size: "M",
    price: 28000,
    imageUrl: "https://i.postimg.cc/t4Lwp5xS/5-1.png",
    imageArr: [
      "https://i.postimg.cc/YSYV9WNp/5-2.png",
      "https://i.postimg.cc/zBkcwYP6/5-3.png",
      "https://via.placeholder.com/150",
    ],
    isSales: true,
    options: ["배송비포함", "새상품"],
      seller: {
        sellerId: "seller6",
        userName: "bmenhenitt5",
        nickname: "chandscombe5",
        userAvatar:
          "https://robohash.org/quisconsequatureveniet.png?size=50x50&set=set1",
        address: "9 Bobwhite Avenue",
        phone: "010-5325-6919",
      },
    reviews: [
      {
        reviewId: "reviewId_1880e",
        reviewInfo: {
          userId: "userId_4925o",
          userName: "ccaveill6",
          userAvatar:
            "https://robohash.org/quasomnissoluta.png?size=70x70&set=set1",
          rating: 2,
          review:
            "Donec diam neque, vestibulum eget, vulputate ut, ultrices vel, augue. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Donec pharetra, magna vestibulum aliquet ultrices, erat tortor sollicitudin mi, sit amet lobortis sapien sapien non mi. Integer ac neque.",
          createdAt: "2024-02-06",
        },
      },
    ],
  },
  {
    itemId: "itemId072c",
    itemName: "NASDAQ",
    description:
      "c Cras in purus eu magna vulputate luctus. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Vivamus vestibulum sagittis sapien. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Etiam vel augue. Vestibulum rutrum rutrum neque. Aenean auctor gravida sem. Praesent id massa id nisl venenatis lacinia. Aenean sit amet justo.",
    quantity: 3,
    size: "L",
    price: 93000,
    imageUrl: "https://i.postimg.cc/PJrF2Vgx/6-1.png",
    imageArr: [
      "https://i.postimg.cc/2yc9JFYx/6-2.png",
      "https://i.postimg.cc/pVqSpxjn/6-3.png",
      "https://i.postimg.cc/28yKWJd9/6-4.png",
    ],
    isSales: true,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller6",
      userName: "bmenhenitt5",
      nickname: "chandscombe5",
      userAvatar:
        "https://robohash.org/quisconsequatureveniet.png?size=50x50&set=set1",
      address: "9 Bobwhite Avenue",
      phone: "010-5325-6919",
    },
    reviews: [
      {
        reviewId: "reviewId_3818H",
        reviewInfo: {
          userId: "userId_4763b",
          userName: "learp7",
          userAvatar:
            "https://robohash.org/vitaequivoluptas.png?size=70x70&set=set1",
          rating: 1,
          review:
            "Curabitur convallis. Duis consequat dui nec nisi volutpat eleifend. Donec ut dolor. Morbi vel lectus in quam fringilla rhoncus.",
          createdAt: "2024-07-13",
        },
      },
      {
        reviewId: "reviewId_6289z",
        reviewInfo: {
          userId: "userId_7992g",
          userName: "camiss8",
          rating: 1,
          review:
            "Phasellus in felis. Donec semper sapien a libero. Nam dui. Proin leo odio, porttitor id, consequat in, consequat ut, nulla. Sed accumsan felis. Ut at dolor quis odio consequat varius. Integer ac leo.",
          createdAt: "2024-09-17",
        },
      },
    ],
  },
  {
    itemId: "itemId835E",
    itemName: "c",
    description:
      "Morbi non lectus. Aliquam sit amet diam in magna bibendum imperdiet. Nullam orci pede, venenatis non, sodales sed, tincidunt eu, felis. Fusce posuere felis sed lacus. Morbi sem mauris, laoreet ut, rhoncus aliquet, pulvinar sed, nisl. Nunc rhoncus dui vel sem. Sed sagittis. Nam congue, risus semper porta volutpat, quam pede lobortis ligula, sit amet eleifend pede libero quis orci. Nullam molestie nibh in lectus. Pellentesque at nulla. Suspendisse potenti. Cras in purus eu magna vulputate luctus. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Vivamus vestibulum sagittis sapien. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Etiam vel augue. Vestibulum rutrum rutrum neque. Aenean auctor gravida sem. Praesent id massa id nisl venenatis lacinia. Aenean sit amet justo.",
    quantity: 1,
    size: "3XL",
    price: 241000,
    imageUrl: "https://i.postimg.cc/zXQPFj0z/7-1.png",
    imageArr: [
      "https://i.postimg.cc/mgFpXH5Q/7-2.png",
      "https://i.postimg.cc/0NRWLyTh/7-3.png",
      "https://i.postimg.cc/LstCRMR5/7-4.png",
    ],
    isSales: true,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller6",
      userName: "bmenhenitt5",
      nickname: "chandscombe5",
      userAvatar:
        "https://robohash.org/quisconsequatureveniet.png?size=50x50&set=set1",
      address: "9 Bobwhite Avenue",
      phone: "010-5325-6919",
    },
    reviews: [
      {
        reviewId: "reviewId_7391G",
        reviewInfo: {
          userId: "userId_8877m",
          userName: "bgaskoin9",
          userAvatar:
            "https://robohash.org/autharumnon.png?size=70x70&set=set1",
          rating: 2,
          review:
            "Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Etiam vel augue. Vestibulum rutrum rutrum neque.",
          createdAt: "2024-08-04",
        },
      },
    ],
  },
  {
    itemId: "itemId744r",
    itemName: "NYSE",
    description:
      "Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Nulla dapibus dolor vel est. Donec odio justo, sollicitudin ut, suscipit a, feugiat et, eros. Vestibulum ac est lacinia nisi venenatis tristique.",
    quantity: 1,
    size: "XL",
    price: 49000,
    imageUrl: "https://i.postimg.cc/bJLmDHZj/8-1.png",
    imageArr: [
      "https://i.postimg.cc/1XtJpnFV/8-2.png",
      "https://i.postimg.cc/qBbmpwfs/8-3.png",
      "https://i.postimg.cc/3JktNG7P/8-4.png",
    ],
    isSales: true,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller7",
      userName: "gdulwitch6",
      nickname: "ldusey6",
      userAvatar:
        "https://robohash.org/noninventoresoluta.png?size=50x50&set=set1",
      address: "426 Forest Terrace",
      phone: "010-8976-7693",
    },
    reviews: [
      {
        reviewId: "reviewId_9291C",
        reviewInfo: {
          userId: "userId_8443j",
          userName: "rmortena",
          userAvatar:
            "https://robohash.org/etliberosunt.png?size=70x70&set=set1",
          rating: 3,
          review: "Suspendisse accumsan tortor quis turpis.",
          createdAt: "2024-05-26",
        },
      },
    ],
  },
  {
    itemId: "itemId727k",
    itemName: "NASDAQ",
    description:
      "Donec quis orci eget orci vehicula condimentum. Curabitur in libero ut massa volutpat convallis. Morbi odio odio, elementum eu, interdum eu, tincidunt in, leo. Maecenas pulvinar lobortis est. Phasellus sit amet erat.",
    quantity: 2,
    size: "XL",
    price: 11000,
    imageUrl: "https://i.postimg.cc/136McZ8B/9-1.png",
    imageArr: [
      "https://i.postimg.cc/yNHjLr9H/9-2.png",
      "https://i.postimg.cc/N0k4S77r/9-3.png",
      "https://i.postimg.cc/pTVZHhqc/9-4.png",
    ],
    isSales: true,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller8",
      userName: "hsavill7",
      nickname: "arathbourne7",
      userAvatar:
        "https://robohash.org/repellendusquidemquisquam.png?size=50x50&set=set1",
      address: "35891 Forest Run Way",
      phone: "010-3819-1895",
    },
    reviews: [
      {
        reviewId: "reviewId_3442O",
        reviewInfo: {
          userId: "userId_9243Y",
          userName: "rbusstb",
          userAvatar:
            "https://robohash.org/eligendivitaedistinctio.png?size=70x70&set=set1",
          rating: 3,
          review:
            "Nulla neque libero, convallis eget, eleifend luctus, ultricies eu, nibh. Quisque id justo sit amet sapien dignissim vestibulum. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Nulla dapibus dolor vel est.",
          createdAt: "2024-06-28",
        },
      },
      {
        reviewId: "reviewId_9792f",
        reviewInfo: {
          userId: "userId_5221R",
          userName: "cwharfi",
          userAvatar: "https://robohash.org/sitadminus.png?size=70x70&set=set1",
          rating: 5,
          review:
            "Nulla neque libero, convallis eget, eleifend luctus, ultricies eu, nibh. Quisque id justo sit amet sapien dignissim vestibulum. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Nulla dapibus dolor vel est. Donec odio justo, sollicitudin ut, suscipit a, feugiat et, eros. Vestibulum ac est lacinia nisi venenatis tristique. Fusce congue, diam id ornare imperdiet, sapien urna pretium nisl, ut volutpat sapien arcu sed augue. Aliquam erat volutpat.",
          createdAt: "2024-05-24",
        },
      },
    ],
  },
  {
    itemId: "itemId405M",
    itemName: "NASDAQ",
    description:
      "Duis at velit eu est congue elementum. In hac habitasse platea dictumst.",
    quantity: 1,
    size: "XS",
    price: 336000,
    imageUrl: "https://i.postimg.cc/wvWkTqrq/10-1.png",
    imageArr: [
      "https://i.postimg.cc/D08PWVVq/10-2.png",
      "https://i.postimg.cc/J0vcZTmS/10-3.png",
      "https://i.postimg.cc/Y2sx5Sg8/10-4.png",
    ],
    isSales: false,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller9",
      userName: "sbisley8",
      nickname: "hhirtzmann8",
      userAvatar:
        "https://robohash.org/veroinventoreblanditiis.png?size=50x50&set=set1",
      address: "17050 Lakewood Parkway",
      phone: "010-7354-2746",
    },
    reviews: [
      {
        reviewId: "reviewId_1577V",
        reviewInfo: {
          userId: "userId_1961W",
          userName: "emcphelimeyc",
          userAvatar:
            "https://robohash.org/earerumdolor.png?size=70x70&set=set1",
          rating: 4,
          review:
            "Cras mi pede, malesuada in, imperdiet et, commodo vulputate, justo. In blandit ultrices enim. Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Proin interdum mauris non ligula pellentesque ultrices. Phasellus id sapien in sapien iaculis congue.",
          createdAt: "2024-07-21",
        },
      },
      {
        reviewId: "reviewId_9308v",
        reviewInfo: {
          userId: "userId_7480s",
          userName: "bgrangierh",
          userAvatar:
            "https://robohash.org/quisidrepudiandae.png?size=70x70&set=set1",
          rating: 4,
          review:
            "Nam nulla. Integer pede justo, lacinia eget, tincidunt eget, tempus vel, pede.",
          createdAt: "2024-03-06",
        },
      },
      {
        reviewId: "reviewId_2022Y",
        reviewInfo: {
          userId: "userId_3948t",
          userName: "lquand",
          userAvatar:
            "https://robohash.org/autvoluptatemomnis.png?size=70x70&set=set1",
          rating: 2,
          review:
            "Vivamus in felis eu sapien cursus vestibulum. Proin eu mi. Nulla ac enim. In tempor, turpis nec euismod scelerisque, quam turpis adipiscing lorem, vitae mattis nibh ligula nec sem.",
          createdAt: "2024-08-27",
        },
      },
    ],
  },
  {
    itemId: "itemId224o",
    itemName: "NYSE",
    description:
      "Aenean lectus. Pellentesque eget nunc. Donec quis orci eget orci vehicula condimentum. Curabitur in libero ut massa volutpat convallis. Morbi odio odio, elementum eu, interdum eu, tincidunt in, leo. Maecenas pulvinar lobortis est. Phasellus sit amet erat. Nulla tempus. Vivamus in felis eu sapien cursus vestibulum. Proin eu mi. Nulla ac enim. In tempor, turpis nec euismod scelerisque, quam turpis adipiscing lorem, vitae mattis nibh ligula nec sem. Duis aliquam convallis nunc. Proin at turpis a pede posuere nonummy.",
    quantity: 3,
    size: "3XL",
    price: 225000,
    imageUrl: "https://i.postimg.cc/pLXKGhXs/11-1.png",
    imageArr: [
      "https://i.postimg.cc/Z57p1bW4/11-2.png",
      "https://i.postimg.cc/FKY3vMSc/11-3.png",
      "https://i.postimg.cc/bv4bwm05/11-4.png",
    ],
    isSales: true,
    options: ["배송비포함", "새상품"],
    seller: {
      sellerId: "seller10",
      userName: "jowttrim9",
      nickname: "mriveles9",
      userAvatar: "https://robohash.org/idoccaecatiqui.png?size=50x50&set=set1",
      address: "650 Bartillon Terrace",
      phone: "010-1860-5163",
    },
    reviews: [
      {
        reviewId: "reviewId_3623z",
        reviewInfo: {
          userId: "userId_3331j",
          userName: "elandelle",
          userAvatar:
            "https://robohash.org/cupiditateida.png?size=70x70&set=set1",
          rating: 3,
          review:
            "Vestibulum sed magna at nunc commodo placerat. Praesent blandit.",
          createdAt: "2024-02-12",
        },
      },
      {
        reviewId: "reviewId_0571e",
        reviewInfo: {
          userId: "userId_7552r",
          userName: "vtomczynskif",
          userAvatar:
            "https://robohash.org/estdoloremquia.png?size=70x70&set=set1",
          rating: 4,
          review:
            "Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Duis faucibus accumsan odio. Curabitur convallis. Duis consequat dui nec nisi volutpat eleifend.",
          createdAt: "2024-01-02",
        },
      },
      {
        reviewId: "reviewId_1680b",
        reviewInfo: {
          userId: "userId_2502a",
          userName: "wklehyng",
          userAvatar:
            "https://robohash.org/cumexcepturitempore.png?size=70x70&set=set1",
          rating: 1,
          review:
            "Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Mauris viverra diam vitae quam. Suspendisse potenti. Nullam porttitor lacus at turpis.",
          createdAt: "2024-09-18",
        },
      },
    ],
  },
];
